       	  @csrf
   											 ...
   											 {{-- <div class="form-group row">
											 	...
											 	<div class="form-group row">
													...
													<label class="col-md-3 form-label">Nom client</label>
															<div class="col-md-9">
																<select disabled class="form-control select2-show-search @error('client_id') is-invalid @enderror" name="client_id" data-placeholder="Choose one (with searchbox)">
																	{{-- <optgroup label="Client">
																		@foreach($clients as $client)														
																			<option value="{{$client->id}}" {{ $client->id ? 'selected' : '' }}>{{ $client->nomcli }}</option>
																		@endforeach
																	</optgroup>							 --}}
																		<option value="{{$purchase->client_id}}">{{$purchase->client->nomcli }}</option>
																</select>
																	@error('client_id"')
																		<div class="invalid-feedback">
																			{{ $errors->first('client_id"') }}
													...
													<div class="form-group row">
														<label class="col-md-3 form-label">Date</label>
														<div class="col-md-9">
															<input disabled value="{{ old('datepurchase') ?? $purchase->datepurchase }}" class="form-control @error('datepurchase') is-invalid @enderror" type="date" placeholder="Entrez....." id="datepurchase" name="datepurchase">
                                                               @error('datepurchase')
                                                                <div class="invalid-feedback">
                                                                {{ $errors->first('datepurchase') }}
                                                                </div>
												...
												{{--	<div class="form-group row">
											 	...
											 	<div class="form-group row">
														<label class="col-md-3 form-label">Ap_id</label>
														<div class="col-md-9">
															<select disabled class="form-control custom-select select2" @error('appro_id') is-invalid @enderror name="appro_id">
																		{{-- @foreach($appros as $appro)
																		<option value="{{$appro->id}}" {{ $appro->id ? 'selected' : '' }}>Id: {{ $appro->id }}, {{ $appro->produit->marque }} {{ $appro->produit->marque }} {{ $appro->produit->model }}</option>
																		@endforeach --}}
																		<option value="{{$purchase->appro_id}}">{{$purchase->appro->produit->marque}} {{$purchase->appro->produit->model}} {{$purchase->appro->produit->caractere}}</option>
																	</select>
															@error('appro_id"')
																<div class="invalid-feedback">
																	{{ $errors->first('appro_id"') }}
